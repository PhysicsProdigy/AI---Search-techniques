...
def show_graph(graph_data, node_colors=None):
    ...
    nx.draw(G, pos={k: node_positions[k] for k in G.nodes()},
            node_color=[node_colors[node] for node in G.nodes()], linewidths=0.3, edgecolors='k', font_size=7)

    # draw labels for nodes only if its all white or if it has been coloured.
    SG = G.__class__()
    for node in G.nodes:
    ...
    if all_white:
        ...
        nx.draw_networkx_labels(SG, pos=node_positions, font_size=8)
